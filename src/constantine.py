...
def build_constant_time(input, output, taint_log, loop_log, optimization_level):
    ...
    PRE_CFL = False
    if PRE_CFL:
        # pre-linearize Control Flow
        opt_exec(f'-cfl -cfl-funcs=__cfl_.* -cfl-protect-branches=1 -cfl-no-wrap=1 -simplifycfg -dce -constprop -o {name}.pre_cfl.bc {name}.linked_2.bc')

        # linearize Data Flow
        opt_exec(f'-dfl -dfl-funcs=.* -dfl-cfl-funcs=__cfl_.* -dfl-avx512=1 -stat=0 -modelConsts -allow-fi-prom=0 -fieldlimit=4294967295 -o {name}.dfl.bc {name}.pre_cfl.bc')
    else:
        # linearize Data Flow
        opt_exec(f'-dfl -dfl-funcs=.* -dfl-cfl-funcs=__cfl_.* -dfl-avx512=1 -stat=0 -modelConsts -allow-fi-prom=0 -fieldlimit=4294967295 -o {name}.dfl.bc {name}.linked_2.bc')
