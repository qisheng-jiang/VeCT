...
ROOT=/app/src
FILE=`pwd`/vector-t-test-false-depen.c
FILE_NAME=${FILE::-2}
...
function run_test {
    ...
    echo "Running test with update_size=$update_size for $test_type with is_load=$is_load"
    cd $ROOT
    . ./setup.sh 
    ...
    cp $FILE_NAME.out $output_dir/$test_type.$update_size.$is_load.final.out
...
do 
    for is_load in 0 1
    do 
        run_test $test_type $update_size $is_load $output_file
    done
done
