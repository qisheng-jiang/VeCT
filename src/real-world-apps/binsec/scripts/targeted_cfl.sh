...
if [ "$PRE_CFL" == "1" ]; then
  ...
  ../opt -cfl -cfl-funcs='__cfl_.*' -cfl-protect-branches=1 -cfl-no-wrap=1 -simplifycfg	-dce -constprop -o $PROJECT_NAME.pre_cfl.bc $PROJECT_NAME.linked_2.bc
  ../opt -dfl -dfl-funcs='.*' -dfl-cfl-funcs='__cfl_.*' $DFL_AVX -dfl-induction-vars=1 -stat=0 -modelConsts -allow-fi-prom=0 -fieldlimit=4294967295 -o $PROJECT_NAME.dfl.bc $PROJECT_NAME.pre_cfl.bc
else
