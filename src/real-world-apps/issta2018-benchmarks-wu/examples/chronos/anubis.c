			 ...
			 unsigned int key_len)
{
	 ...
	 * 				  K^0, K'^r = theta(K^{R-r}):
	 */
	ctx->D[0 * 4 + 0] = ctx->E[14 * 4 + 0];
	ctx->D[14 * 4 + 0] = ctx->E[0 * 4 + 0];
	ctx->D[0 * 4 + 1] = ctx->E[14 * 4 + 1];
	ctx->D[14 * 4 + 1] = ctx->E[0 * 4 + 1];
	ctx->D[0 * 4 + 2] = ctx->E[14 * 4 + 2];
	ctx->D[14 * 4 + 2] = ctx->E[0 * 4 + 2];
	ctx->D[0 * 4 + 3] = ctx->E[14 * 4 + 3];
	ctx->D[14 * 4 + 3] = ctx->E[0 * 4 + 3];

	#pragma unroll
	for (r = 1; r < 14; r++) {
	    #pragma unroll
		for (i = 0; i < 4; i++) {
			uint32_t v = ctx->E[(R - r) * 4 + i];
			uint32_t t_0, t_1, t_2, t_3; 
			t_0 = T4[(v >> 24)       ];
			t_1 = T4[(v >> 16) & 0xff];
			t_2 = T4[(v >>  8) & 0xff];
			t_3 = T4[(v      ) & 0xff];
...
static uint8_t out[TEST_DATA_SIZE] = {0x00};
