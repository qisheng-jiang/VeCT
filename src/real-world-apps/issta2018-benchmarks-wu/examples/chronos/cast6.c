...
#define rol32(word,shift) ((shift)?((word) << (shift)) | ((word) >> (32 - (shift))):(word))
...
/* forward octave */
static inline __attribute__((always_inline))void W(uint32_t *key, unsigned int i)
{
	uint32_t I=0;
...
int cast6_setkey(const uint8_t *in_key, struct cast6_ctx *c, 
			unsigned key_len)
{
	int i;
	uint32_t key[8] = {0};
...
/*forward quad round*/
static inline __attribute__((always_inline)) void Q(uint32_t *block, uint32_t *Kr, uint32_t *Km)
{
	uint32_t I;
	uint32_t kr_0, kr_1, kr_2, kr_3;
	uint32_t km_0, km_1, km_2, km_3;
	kr_0 = Kr[0]; kr_1 = Kr[1]; kr_2 = Kr[2]; kr_3 = Kr[3];
	km_0 = Km[0]; km_1 = Km[1]; km_2 = Km[2]; km_3 = Km[3];
	block[2] ^= F1(block[3], kr_0, km_0);
	block[1] ^= F2(block[2], kr_1, km_1);
	block[0] ^= F3(block[1], kr_2, km_2);
	block[3] ^= F1(block[0], kr_3, km_3);
}

/*reverse quad round*/
static inline  __attribute__((always_inline)) void QBAR(uint32_t *block, uint32_t *Kr, uint32_t *Km)
{
	uint32_t I;
	uint32_t kr_0, kr_1, kr_2, kr_3;
	uint32_t km_0, km_1, km_2, km_3;
