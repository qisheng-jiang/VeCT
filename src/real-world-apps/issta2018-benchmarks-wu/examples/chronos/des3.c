...
static const uint32_t pc1[256] __attribute__((aligned(64))) = {
...
static const uint32_t rs[256] __attribute__((aligned(64))) = {
...
unsigned long des_ekey(const uint8_t *k, uint32_t *pe)
{
	...
	d = k[4]; d &= 0x0e; d <<= 4; d |= k[0] & 0x1e;
	c = k[5]; c &= 0x0e; c <<= 4; c |= k[1] & 0x1e;
	b = k[6]; b &= 0x0e; b <<= 4; b |= k[2] & 0x1e;
	a = k[7]; a &= 0x0e; a <<= 4; a |= k[3] & 0x1e;
	...
	d = k[0]; d &= 0xe0; d >>= 4; d |= k[4] & 0xf0; 
	c = k[1]; c &= 0xe0; c >>= 4; c |= k[5] & 0xf0; 
	b = k[2]; b &= 0xe0; b >>= 4; b |= k[6] & 0xf0; 
	a = k[3]; a &= 0xe0; a >>= 4; a |= k[7] & 0xf0; 
 ...
 *
 */
inline __attribute__((always_inline)) int des3_ede_setkey(const uint8_t *key, struct des3_ede_ctx *dctx, unsigned int keylen) 
{
	const uint32_t *K = (const uint32_t *)key;
	uint32_t *expkey = dctx->expkey;

	des_ekey(key, expkey); expkey += DES_EXPKEY_WORDS; key += DES_KEY_SIZE;
	dkey(key, expkey); expkey += DES_EXPKEY_WORDS; key += DES_KEY_SIZE;
	des_ekey(key, expkey);
...
static uint8_t in[TEST_DATA_SIZE]  __attribute__((aligned(64)))= {0x00};
static uint8_t out[TEST_DATA_SIZE]  __attribute__((aligned(64)))= {0};
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < TEST_DATA_SIZE; i += 64)
		des3_ede_encrypt(&ctx, out + i, in + i);
#if END_TO_END != 1
        _mm_mfence();
        end = __rdtscp(&temp);
