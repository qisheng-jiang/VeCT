...
# define PUTuint32_t(ct, st) {*((uint32_t *)(ct)) = SWAP((st));}
...
/* Functions for loading and storing unaligned uint32_t values of different
   endianness.  */
inline __attribute__((always_inline)) uint32_t buf_get_be32(const void *_buf)
{
...
inline __attribute__((always_inline)) void buf_put_be32(void *_buf, uint32_t val)
{
  uint8_t *out = _buf;
  out[0] = val >> 24;
  out[1] = val >> 16;
...
{   
    ...
    for (uint64_t i = 0; i < WARMUP_COUNT + REPEAT_COUNT; i++) {
        ...
        clean_cache(subkey, sizeof(subkey));
        _mm_mfence();
        start = __rdtscp(&temp);
#endif
...
//     krl  = GETuint32_t(key +  8);
//     krr  = GETuint32_t(key + 12);
//     krll = GETuint32_t(key + 16);
//     krlr = GETuint32_t(key + 20);
//     krrl = GETuint32_t(key + 24);
//     krrr = GETuint32_t(key + 28);
...
//  *
//  * "io" must be 4uint8_t aligned and big-endian data.
//  */
...
//     tmp[0] = GETuint32_t(plaintext);
//     tmp[1] = GETuint32_t(plaintext + 4);
//     tmp[2] = GETuint32_t(plaintext + 8);
//     tmp[3] = GETuint32_t(plaintext + 12);
...
//     PUTuint32_t(ciphertext, tmp[0]);
//     PUTuint32_t(ciphertext + 4, tmp[1]);
//     PUTuint32_t(ciphertext + 8, tmp[2]);
//     PUTuint32_t(ciphertext + 12, tmp[3]);
// }
...
//     tmp[0] = GETuint32_t(ciphertext);
//     tmp[1] = GETuint32_t(ciphertext + 4);
//     tmp[2] = GETuint32_t(ciphertext + 8);
//     tmp[3] = GETuint32_t(ciphertext + 12);
...
// 	break;
//     }
//     PUTuint32_t(plaintext, tmp[0]);
//     PUTuint32_t(plaintext + 4, tmp[1]);
//     PUTuint32_t(plaintext + 8, tmp[2]);
//     PUTuint32_t(plaintext + 12, tmp[3]);
// }
// #endif /*!USE_ARM_ASM*/
