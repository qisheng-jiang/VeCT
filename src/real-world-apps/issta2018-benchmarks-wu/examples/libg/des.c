...
// static int des_ecb_crypt (struct _des_ctx *, const uint8_t *, uint8_t *, int);
 ...
 */
static uint32_t encrypt_rotate_tab[16] __attribute__((aligned(64)))=
{
 ...
 */
static uint32_t weak_keys[64 * 8] __attribute__((aligned(64)))=
{
...
des_key_schedule (const uint8_t * rawkey, uint32_t * subkey) 
{
  uint32_t left, right, work;
  uint32_t key_swap_0, key_swap_1, key_swap_2, key_swap_3, 
           key_swap_4, key_swap_5, key_swap_6, key_swap_7;
  int round;
  ...
  key_swap_0 = leftkey_swap[(left >> 0) & 0xf];
  key_swap_1 = leftkey_swap[(left >> 8) & 0xf];
  key_swap_2 = leftkey_swap[(left >> 16) & 0xf];
  key_swap_3 = leftkey_swap[(left >> 24) & 0xf];
  key_swap_4 = leftkey_swap[(left >> 5) & 0xf];
  key_swap_5 = leftkey_swap[(left >> 13) & 0xf];
  key_swap_6 = leftkey_swap[(left >> 21) & 0xf];
  key_swap_7 = leftkey_swap[(left >> 29) & 0xf];
  ...
  key_swap_0 = rightkey_swap[(right >> 1) & 0xf];
  key_swap_1 = rightkey_swap[(right >> 9) & 0xf];
  key_swap_2 = rightkey_swap[(right >> 17) & 0xf];
  key_swap_3 = rightkey_swap[(right >> 25) & 0xf];
  key_swap_4 = rightkey_swap[(right >> 4) & 0xf];
  key_swap_5 = rightkey_swap[(right >> 12) & 0xf];
  key_swap_6 = rightkey_swap[(right >> 20) & 0xf];
  key_swap_7 = rightkey_swap[(right >> 28) & 0xf];
 ...
 */
static int des_setkey (const uint8_t * key, struct _des_ctx *ctx)
{
 ...
 */
static int
des_ecb_crypt (struct _des_ctx *ctx, const uint8_t * from, uint8_t * to, int mode)
{
  uint32_t left, right, work;
  uint32_t *keys;
...
static inline void
tripledes_amd64_ctr_enc(const void *keys, uint8_t *out, const uint8_t *in, uint8_t *ctr)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
tripledes_amd64_cbc_dec(const void *keys, uint8_t *out, const uint8_t *in, uint8_t *iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
tripledes_amd64_cfb_dec(const void *keys, uint8_t *out, const uint8_t *in, uint8_t *iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
//    */
//   {
//     static const struct { uint8_t key[24]; uint8_t plain[8]; uint8_t cipher[8]; }
//       testdata[] = {
...
static uint8_t in[TEST_DATA_SIZE] = {0x00};
...
{ 
    ...
    for (int i = 0; i < TEST_DATA_SIZE; i += 64)
        des_ecb_crypt(ctx, in + i, out + i, MODE_ENC);
#if END_TO_END != 1
        _mm_mfence();
        end = __rdtscp(&temp);
