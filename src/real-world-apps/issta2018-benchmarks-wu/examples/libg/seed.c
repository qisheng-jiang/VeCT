 ...
 */
static int
do_setkey (const uint8_t *key, SEED_context *ctx, const unsigned keylen)
{
  ...
  int i;
  uint32_t s0_0, s0_1, s1_0, s1_1, s2_0, s2_1, s3_0, s3_1;
  ...
  for (i = 0; i < NUMKC; i++)
  {
      ...
      s3_0 = SS3[t0.b3];
      s3_1 = SS3[t1.b3];
      *(keyout++) = s0_0 ^ s1_0 ^ s2_0 ^ s3_0;
      *(keyout++) = s0_1 ^ s1_1 ^ s2_1 ^ s3_1;
...
static void
do_encrypt (const SEED_context *ctx, uint8_t *outbuf, const uint8_t *inbuf)
{
  uint32_t x1, x2, x3, x4;
  union wordbuf t0, t1;
...
static void
do_decrypt (SEED_context *ctx, uint8_t *outbuf, const uint8_t *inbuf)
{
  uint32_t x1, x2, x3, x4;
  union wordbuf t0, t1;
...
static uint8_t in_key[16] = { 0x47, 0x06, 0x48, 0x08, 0x51, 0xE6, 0x1B, 0xE8, 0x5D, 0x74, 0xBF, 0xB3, 0xFD, 0x95, 0x61, 0x85};
