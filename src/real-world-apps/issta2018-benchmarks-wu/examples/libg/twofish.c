...
/* Functions for loading and storing unaligned uint32_t values of different
   endianness.  */
static inline __attribute__((always_inline)) uint32_t buf_get_le32(const void *_buf) 
{
...
static inline __attribute__((always_inline)) void buf_put_le32(void *_buf, uint32_t val) 
{
  uint8_t *out = _buf;
  out[3] = val >> 24;
  out[2] = val >> 16;
...
static inline __attribute__((always_inline)) uint32_t buf_get_be32(const void *_buf) 
{
  const uint8_t *in = _buf;
...
static inline __attribute__((always_inline)) void buf_put_be32(void *_buf, uint32_t val) 
{
  uint8_t *out = _buf;
  out[0] = val >> 24;
  out[1] = val >> 16;
...
static const uint32_t q0[256] __attribute__((aligned(64)))= {
...
static const uint32_t q1[256] __attribute__((aligned(64)))= {
...
static const uint32_t poly_to_exp[256] __attribute__((aligned(64)))= {
   492,
...
static const uint32_t exp_to_poly[492 + 256] __attribute__((aligned(64)))= {
...
static uint32_t calc_sb_tbl[512] __attribute__((aligned(64)))= {
...
#define CALC_SB_2(i, a, b) \
   uint32_t tmp_q0_0 = q0[(a) ^ sa]; \
   uint32_t tmp_q0_1 = q0[(b) ^ sb]; \
   uint32_t tmp_q1_0 = q1[(a) ^ sc]; \
   uint32_t tmp_q1_1 = q1[(b) ^ sd]; \
   tmp_q0_0 = mds[0 * 256 + (tmp_q0_0 ^ se)]; \
   tmp_q0_1 = mds[1 * 256 + (tmp_q0_1 ^ sf)]; \
   tmp_q1_0 = mds[2 * 256 + (tmp_q1_0 ^ sg)]; \
   tmp_q1_1 = mds[3 * 256 + (tmp_q1_1 ^ sh)]; \
   ctx->s[0 * 256 + i] = tmp_q0_0; \
   ctx->s[1 * 256 + i] = tmp_q0_1; \
   ctx->s[2 * 256 + i] = tmp_q1_0; \
   ctx->s[3 * 256 + i] = tmp_q1_1;
...
#define CALC_SB256_2(i, a, b) \
   ctx->s[0 * 256 + i] = mds[0 * 256 + (q0[q0[q1[(b) ^ sa] ^ se] ^ si] ^ sm)]; \
   ctx->s[1 * 256 + i] = mds[1 * 256 + (q0[q1[q1[(a) ^ sb] ^ sf] ^ sj] ^ sn)]; \
   ctx->s[2 * 256 + i] = mds[2 * 256 + (q1[q0[q0[(a) ^ sc] ^ sg] ^ sk] ^ so)]; \
   ctx->s[3 * 256 + i] = mds[3 * 256 + (q1[q1[q0[(b) ^ sd] ^ sh] ^ sl] ^ sp)];
...
#define CALC_K_2(a, b, c, d, j) \
     mds[0 * 256 + (q0[a ^ key[(j) + 8]] ^ key[j])] \
   ^ mds[1 * 256 + (q0[b ^ key[(j) + 9]] ^ key[(j) + 1])] \
   ^ mds[2 * 256 + (q1[c ^ key[(j) + 10]] ^ key[(j) + 2])] \
   ^ mds[3 * 256 + (q1[d ^ key[(j) + 11]] ^ key[(j) + 3])]

#define CALC_K_2_RET(R, a, b, c, d, j) \
    tmp_key_0 = key[j]; \
    tmp_key_1 = key[(j) + 1]; \
    tmp_key_2 = key[(j) + 2]; \
    tmp_key_3 = key[(j) + 3]; \
    ...
    tmp_key_10 = key[(j) + 10]; \
    tmp_key_11 = key[(j) + 11]; \
    tmp_q0_a = q0[a ^ tmp_key_8]; \
    tmp_q0_b = q0[b ^ tmp_key_9]; \
    tmp_q1_c = q1[c ^ tmp_key_10]; \
    tmp_q1_d = q1[d ^ tmp_key_11]; \
    R = mds[0 * 256 + (tmp_q0_a ^ tmp_key_0)] \
   ^ mds[1 * 256 + (tmp_q0_b ^ tmp_key_1)] \
   ^ mds[2 * 256 + (tmp_q1_c ^ tmp_key_2)] \
   ^ mds[3 * 256 + (tmp_q1_d ^ tmp_key_3)]

#define CALC_K(a, j, k, l, m, n) \
   uint32_t tmp_key_0, tmp_key_1, tmp_key_2, tmp_key_3, \
			tmp_key_8, tmp_key_9, tmp_key_10, tmp_key_11, \
			tmp_q0_a, tmp_q0_b, tmp_q1_c, tmp_q1_d; \
   CALC_K_2_RET (x, k, l, k, l, 0); \
   CALC_K_2_RET (y, m, n, m, n, 4); \
   y = (y << 8) + (y >> 24); \
   x += y; y += x; ctx->a[j] = x; \
   ctx->a[(j) + 1] = (y << 9) + (y >> 23)
...
do_twofish_setkey (const uint8_t *key, TWOFISH_context *ctx, const unsigned keylen)
{
  ...
  // else
  {
      ...
      for(i=j=0,k=1; i < 256; i++, j += 2, k += 2 )
      {
        uint32_t c_j = calc_sb_tbl[j];
        uint32_t c_k = calc_sb_tbl[k];
        CALC_SB_2( i, c_j, c_k );
      }
      ...
      for (i = 0; i < 8; i += 2)
    	{
        uint32_t q0_0 = q0[i];
        uint32_t q0_1 = q0[i + 1];
        uint32_t q1_0 = q1[i];
        uint32_t q1_1 = q1[i + 1];
    	  CALC_K ( w, i, q0_0, q1_0, q0_1, q1_1 );
    	}
      #pragma unroll
      for (j = 0; j < 32; j += 2, i += 2)
    	{
        ...
        uint32_t q1_0 = q1[i];
        uint32_t q1_1 = q1[i + 1];
    	  CALC_K ( k, j, q0_0, q1_0, q0_1, q1_1 );
    	}
  }
...
extern void _gcry_twofish_amd64_ocb_enc(const TWOFISH_context *ctx, uint8_t *out,
					const uint8_t *in, uint8_t *offset,
					uint8_t *checksum, const u64 Ls[3]);

extern void _gcry_twofish_amd64_ocb_dec(const TWOFISH_context *ctx, uint8_t *out,
					const uint8_t *in, uint8_t *offset,
					uint8_t *checksum, const u64 Ls[3]);
...
static inline void
twofish_amd64_encrypt_block(const TWOFISH_context *c, uint8_t *out, const uint8_t *in)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
twofish_amd64_decrypt_block(const TWOFISH_context *c, uint8_t *out, const uint8_t *in)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
twofish_amd64_ctr_enc(const TWOFISH_context *c, uint8_t *out, const uint8_t *in,
                      uint8_t *ctr)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
twofish_amd64_cbc_dec(const TWOFISH_context *c, uint8_t *out, const uint8_t *in,
                      uint8_t *iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
twofish_amd64_cfb_dec(const TWOFISH_context *c, uint8_t *out, const uint8_t *in,
                      uint8_t *iv)
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
twofish_amd64_ocb_enc(const TWOFISH_context *ctx, uint8_t *out, const uint8_t *in,
		      uint8_t *offset, uint8_t *checksum, const u64 Ls[3])
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
twofish_amd64_ocb_dec(const TWOFISH_context *ctx, uint8_t *out, const uint8_t *in,
		      uint8_t *offset, uint8_t *checksum, const u64 Ls[3])
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
static inline void
twofish_amd64_ocb_auth(const TWOFISH_context *ctx, const uint8_t *abuf,
		       uint8_t *offset, uint8_t *checksum, const u64 Ls[3])
{
#ifdef HAVE_COMPATIBLE_GCC_WIN64_PLATFORM_AS
...
#define G1(a) \
     (ctx->s[0 * 256 + ((a) & 0xFF)]) ^ (ctx->s[1 * 256 + (((a) >> 8) & 0xFF)]) \
   ^ (ctx->s[2 * 256 + (((a) >> 16) & 0xFF)]) ^ (ctx->s[3 * 256 + ((a) >> 24)])

#define G2(b) \
     (ctx->s[1 * 256 + ((b) & 0xFF)]) ^ (ctx->s[2 * 256 + (((b) >> 8) & 0xFF)]) \
   ^ (ctx->s[3 * 256 + (((b) >> 16) & 0xFF)]) ^ (ctx->s[0 * 256 + ((b) >> 24)])
...
selftest (void)
{
  ...
  static uint8_t plaintext_256[16] = {
...
static uint8_t in[TEST_DATA_SIZE] = {0xC8, 0x23, 0xB8, 0xB7, 0x6B, 0xFE, 0x91, 0x13, 0x2F, 0xA7, 0x5E, 0xE6, 0x94, 0x77, 0x6F, 0x6B};
static uint8_t out[TEST_DATA_SIZE] = {0};

int main(int argc, char *argv[])
{
  TWOFISH_context ctx;     /* Expanded key. */
  read(0, in_key, 16);
  read(0, in, TEST_DATA_SIZE);
#if END_TO_END != 1
    uint64_t start, end;
    uint64_t cpu_time_used = 0;
    int temp = 0;
    for (uint64_t i = 0; i < WARMUP_COUNT + REPEAT_COUNT; i++) {
        clean_cache(in_key, 16);
        clean_cache(in, TEST_DATA_SIZE);
        clean_cache(out, TEST_DATA_SIZE);
        _mm_mfence();
        start = __rdtscp(&temp);
#endif
  do_twofish_setkey (in_key, &ctx, 16);
  for (int i = 0; i < TEST_DATA_SIZE; i += 16)
      do_twofish_encrypt (&ctx, out + i, in + i);
#if END_TO_END != 1
        _mm_mfence();
        end = __rdtscp(&temp);
  ...
  // static const uint8_t test_encrypt[4][16] = {
  ...
  // };
  // static const uint8_t test_decrypt[4][16] = {
  //   {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
  //    0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF},
  //   {0x0F, 0x1E, 0x2D, 0x3C, 0x4B, 0x5A, 0x69, 0x78,
  //    0x87, 0x96, 0xA5, 0xB4, 0xC3, 0xD2 ,0xE1, 0xF0},
