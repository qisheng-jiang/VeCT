...
typedef struct {
    ...
    /* padding to align the struct to 1024 */
    uint8_t pad[948];
    /* Subkeys (S-boxes) */
    uint32_t S1[256];
    uint32_t S2[256];
...
static void Blowfish_init(Blowfish_state *self, const unsigned char *key, int keylen)
{
    ...
    for (i = 0; i < 256; i += SPLIT_VECTOR) {
        self->S1[i] = initial_S1[i];
        self->S1[i+1] = initial_S1[i+1];
        self->S1[i+2] = initial_S1[i+2];
        self->S1[i+3] = initial_S1[i+3];
        self->S1[i+4] = initial_S1[i+4];
        self->S1[i+5] = initial_S1[i+5];
        self->S1[i+6] = initial_S1[i+6];
        self->S1[i+7] = initial_S1[i+7];
    }
    for (i = 0; i < 256; i += SPLIT_VECTOR) {
        self->S2[i] = initial_S2[i];
        self->S2[i+1] = initial_S2[i+1];
        self->S2[i+2] = initial_S2[i+2];
        self->S2[i+3] = initial_S2[i+3];
        self->S2[i+4] = initial_S2[i+4];
        self->S2[i+5] = initial_S2[i+5];
        self->S2[i+6] = initial_S2[i+6];
        self->S2[i+7] = initial_S2[i+7];
    }
    for (i = 0; i < 256; i += SPLIT_VECTOR) {
        self->S3[i] = initial_S3[i];
        self->S3[i+1] = initial_S3[i+1];
        self->S3[i+2] = initial_S3[i+2];
        self->S3[i+3] = initial_S3[i+3];
        self->S3[i+4] = initial_S3[i+4];
        self->S3[i+5] = initial_S3[i+5];
        self->S3[i+6] = initial_S3[i+6];
        self->S3[i+7] = initial_S3[i+7];
    }
    for (i = 0; i < 256; i += SPLIT_VECTOR) {
        self->S4[i] = initial_S4[i];
        self->S4[i+1] = initial_S4[i+1];
        self->S4[i+2] = initial_S4[i+2];
        self->S4[i+3] = initial_S4[i+3];
        self->S4[i+4] = initial_S4[i+4];
        self->S4[i+5] = initial_S4[i+5];
        self->S4[i+6] = initial_S4[i+6];
        self->S4[i+7] = initial_S4[i+7];
    }
