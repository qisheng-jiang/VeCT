...
runonsimple()
{
	...
	echo "${bench_to_lib[$base]}"
	dft_start=$(date +%s%3N)
	LIBBC="${bench_to_lib[$base]}" OPT=$OPT ./scripts/build_dft.sh $file1 $cc "$cflags" "$ldflags"
	dft_end=$(date +%s%3N)
	...
	lin_start=$(date +%s%3N)
	OPT=$OPT DFL_AVX="-dfl-avx512=1" NOAVX="-march=skylake-avx512" AVX_VER=".avx512" ./scripts/targeted_cfl.sh $file1 $cc > stats.txt
	lin_end=$(date +%s%3N)
...
c_benchmarks="bearssl/aes_big_wrapper bearssl/des_tab_wrapper"
declare -A bench_to_lib=(
		["bearssl/aes_big_wrapper"]="bearssl/bearssl.bc" 
		["bearssl/des_tab_wrapper"]="bearssl/bearssl.bc" 
		["bearssl/aes_ct_wrapper"]="bearssl/bearssl.bc" 
		["bearssl/des_ct_wrapper"]="bearssl/bearssl.bc" )
