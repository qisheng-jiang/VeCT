...
# removed -branch-enhance due to bug in CAST:castfunc for 3 sided if generated from lowerswitch
../opt -scalarizer -scalarize-load-store -fix-scalarizer -lowerswitch -lowerinvoke -loop-simplify -mem2reg -remove-unreachable -remove-unreachable-funcs='.*' -remove-selfloops -remove-selfloops-funcs='.*' -remove-dup-lifetimes -mergereturn -fix-irreducible -unify-loop-exits -insert-compares -custom-structurizecfg -internalize -internalize-public-api-list=main -globaldce -check-undefined -o $PROJECT_NAME.cfgnorm.bc $PROJECT_NAME.icp.bc
...
# sanity check
./$PROJECT_NAME.dft.out < random_input.txt | xxd || true
# execute the binary to dump the taint information with all random inputs in 32-bytes chunks
rm dtf.raw || true
split -b128 --filter="./$PROJECT_NAME.dft.out; cat dft.log >> dft.raw" ./random_input.txt >/dev/null
cat dft.raw | sort | uniq > dft.log
rm dft.raw

echo "*** Tainted instructions:" > $LIBNAME.taint
...
paste dft.log dft.symb.2 >> $LIBNAME.taint
